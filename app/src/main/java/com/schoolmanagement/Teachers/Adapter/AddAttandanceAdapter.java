...
public class AddAttandanceAdapter extends RecyclerView.Adapter<AddAttandanceAdapter.ViewHolder> implements Filterable {
     ...
     public AddAttandanceAdapter(Activity activity, List<Attandancelist> items) {
        this.activity = activity;
        this.item= items;
        origList=new ArrayList<>(item);
     }

    @NonNull
    @Override
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.teachers_row_attendance, parent, false);
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, final int position){
        ...
        Attandancelist  a= item.get(position);
        holder.name.setText(a.getName());

        if(a.getPresent().equals("0")){
            holder.present.setChecked(false);
            holder.absent.setChecked(false);
        }else if(a.getPresent().equals("1")) {
